...
test('calculateWeightedAuraScore matches the writing aura formula for a fixed set of countables', function (): void {
    $countables = [
        'likes' => 5,
        'comments' => 2,
        'shelf' => 1,
...
test('calculateWeightedAuraScore matches the user aura formula for a fixed set of countables', function (): void {
    $countables = [
        'writings' => 3,
        'likes' => 5,
        'comments' => 2,
...
test('calculateWeightedAuraScore derives the divisor from the countable count automatically', function (): void {
...
test('calculateWeightedAuraScore returns a zero score when the base is zero', function (): void {
...
test('resolveSort returns the requested sort when it is in the whitelist', function (): void {
    request()->merge(['sort' => 'popular']);

    expect(resolveSort(['latest', 'popular']))->toBe('popular');
});

test('resolveSort falls back to the default when the sort is not whitelisted', function (): void {
